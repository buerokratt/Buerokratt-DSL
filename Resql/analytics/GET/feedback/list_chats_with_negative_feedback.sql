...
WITH
   ...
   rated_chats AS (
       SELECT
           ...
           (
               SELECT
                   ARRAY_AGG(DISTINCT best_name)
                   FILTER (WHERE best_name IS NOT NULL)
               FROM (
                   SELECT DISTINCT ON (dcm_inner.customer_support_id)
                       dcm_inner.customer_support_id,
                       TRIM(
                           CASE
                               WHEN dcm_inner.customer_support_id = 'chatbot'
                   ...
                   ORDER BY 
                       dcm_inner.customer_support_id,
                       CASE WHEN dcm_inner.customer_support_first_name IS NOT NULL AND dcm_inner.customer_support_last_name IS NOT NULL THEN 1 ELSE 2 END,
                       dcm_inner.timestamp DESC
               ) sub
           ) AS all_csa_names,
