...
WHERE date_trunc('day', redirected_chats.updated AT TIME ZONE :timezone) = date_trunc('day', (current_date AT TIME ZONE :timezone - '1 day'::INTERVAL))
UNION ALL
...
WHERE (
  ...
  AND (
    array_length(ARRAY[:urls]::text[], 1) IS NULL
        OR (array_length(ARRAY[:urls]::text[], 1) = 1 AND (ARRAY[:urls]::text[])[1] = 'none')
        OR chat.end_user_url LIKE ANY(ARRAY[:urls]::text[]))
  AND date_trunc('day', chat.ended AT TIME ZONE :timezone) = date_trunc('day', (current_date AT TIME ZONE :timezone - '1 day'::INTERVAL));
