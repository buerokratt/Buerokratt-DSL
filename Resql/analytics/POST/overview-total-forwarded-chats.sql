WITH forwarded_chats_by_csa AS (
    SELECT base_id, customer_support_id, customer_support_display_name, created 
    FROM chat 
    WHERE status = 'REDIRECTED'
    AND customer_support_id <> '' AND customer_support_id IS NOT NULL
    GROUP BY base_id, customer_support_id, customer_support_display_name, created
)
SELECT COUNT(DISTINCT base_id) AS metric_value
FROM forwarded_chats_by_csa
WHERE date_trunc('day', forwarded_chats_by_csa.created) = date_trunc('day', current_date - '1 day'::INTERVAL)
UNION ALL
